...
func (q *Queries) GetMessageThread(ctx context.Context, arg GetMessageThreadParams) ([]Message, error) {
	rows, err := q.db.QueryContext(ctx, getMessageThread, arg.SenderID, arg.ReceiverID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) ListConversations(ctx context.Context, senderID uuid.UUID) ([]Message, error) {
	rows, err := q.db.QueryContext(ctx, listConversations, senderID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
